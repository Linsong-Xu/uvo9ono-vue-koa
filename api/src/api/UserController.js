...
class UserController {
  ...
  async getBasicInfo (ctx) {
    ...
    if (ctx.header.authorization !== undefined && ctx.header.authorization !== '') {
  ...
  // 获取最新签到
  async getNewestSign (ctx) {
    const result = await SignRecord.getNewestSign()
    ctx.body = {
      code: 200,
      data: result
    }
  ...
  // 获取今日最快
  async getEarlyestSign (ctx) {
    const result = await SignRecord.getEarlyestSign()
    ctx.body = {
      code: 200,
      data: result
    }
  ...
  async getTopSign (ctx) {
    const result = await User.getTopSign()
    ctx.body = {
      code: 200,
      data: result
    }
